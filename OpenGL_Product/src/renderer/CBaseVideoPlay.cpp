...
namespace
{
    ...
    // 过滤器类型
    const char *filter_descr="null";    // 不做操作
    const char *filter_mirror = "crop=iw/2:ih:0:0,split[left][tmp];[tmp]hflip[right];[left]pad=iw*2[a];[a][right]overlay=w";    // 左右镜像
    const char *filter_watermark = "movie=./picture/my_logo.png[wm];[in][wm]overlay=5:5[out]";  // 水印添加
    const char *filter_negate = "negate[out]";
    const char *filter_edge = "edgedetect[out]";
    const char *filter_split4 = "scale=iw/2:ih/2[in_tmp];[in_tmp]split=4[in_1][in_2][in_3][in_4];[in_1]pad=iw*2:ih*2[a];[a][in_2]overlay=w[b];[b][in_3]overlay=0:h[d];[d][in_4]overlay=w:h[out]";   // 4分屏显示
...
static int init_filters(const char *filters_descr, AVCodecContext *pCodecCtx, AVFilterGraph **filter_graph, AVFilterContext **buffersrc_ctx, AVFilterContext **buffersink_ctx)
{
    ...
    *filter_graph = avfilter_graph_alloc();
    ...
    if ((ret = avfilter_graph_parse_ptr(*filter_graph, filter_watermark, &inputs, &outputs, NULL)) < 0) {
        exit(1);
    }
...
#ifdef FILTER_TEST
                /* pull filtered pictures from the filtergraph and deal with frame, then push into queue */
                int nCnt = 0;
                while (1) {
                    int ret = av_buffersink_get_frame(buffersink_ctx, pFrame);
